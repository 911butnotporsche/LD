...
%% Set up the training data (SIMULATED METEOROLOGICAL DATA)
% Sintetiniai (simuliuoti) paros temperatūros matavimai, imituojantys
% mobilios meteorologinės stotelės daviklio duomenis: bazinė temperatūra,
% paros ciklas (šilčiau po pietų), lėta atšilimo tendencija per dienas,
% ir atsitiktinis daviklio triukšmas.
rng(0) % reprodukavimui
k = 5; % Dienų skaičius (analogas originalo "periodų" kintamajam)
x = linspace(0, k * 24, 500)'; % Laikas valandomis (0 iki k*24 val.)

T_baseline = 15;      % Bazinė temperatūra, °C
T_amplitude = 8;       % Paros svyravimo amplitudė, °C
T_phase = 9;           % Faziniai poslinkis (piko laikas ~15:00)
T_trend = 0.3;         % Šilimo tendencija, °C/parą
noise_std = 0.5;       % Daviklio triukšmo std. nuokrypis, °C

T_true = T_baseline + T_amplitude*sin(2*pi*(x - T_phase)/24) + T_trend*(x/24);
T_meas = T_true + noise_std*randn(size(x)); % pridėtas matavimo triukšmas

% Normalizavimas į [0,1] intervalą (kaip originaliame pavyzdyje)
y = (T_meas - min(T_meas)) / (max(T_meas) - min(T_meas));

%% Specify the prediction point
xq = 3*24 + 15; % Ketvirtos dienos 15:00 (paros temperatūros pikas)
Tq_true = T_baseline + T_amplitude*sin(2*pi*(xq - T_phase)/24) + T_trend*(xq/24);
yq = (Tq_true - min(T_meas)) / (max(T_meas) - min(T_meas)); % tikslo reikšmė be triukšmo
